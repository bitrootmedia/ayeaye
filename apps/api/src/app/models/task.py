...
# Sorting a list by status has to follow the workflow, not the alphabet:
# "blocker, in_progress, on_hold, review, todo" is an ordering of spellings.
# Board order — the sequence at the top of this file, with the two parked
# states after the ones in flight.
STATUS_RANK = {
    STATUS_TODO: 1,
...
class Task(Base):
    ...
    **Owner and action-required are also two different things.** The owner is
    ...
    # Sanitised HTML — see services/richtext.py. Never rendered without going
    # back through `render()`, which is what puts live URLs on its images.
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    # **Generated by Postgres**, tags stripped, and the only thing search
    # reads. `Computed` is what stops SQLAlchemy trying to write it — and
    # being unwritable is exactly the property that keeps it from drifting
    # away from `description`.
    description_text: Mapped[str | None] = mapped_column(
        Text,
        Computed("regexp_replace(coalesce(description, ''), '<[^>]*>', ' ', 'g')", persisted=True),
        nullable=True,
    )
